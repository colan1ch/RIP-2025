...
func UploadImage(ctx context.Context, client *minio.Client, bucket string, file *multipart.FileHeader, index ds.Index) (string, error) {
	f, err := file.Open()
	if err != nil {
		return "",err
	}
	...
	objectName := fmt.Sprintf("%s%s", index.Name, ext)
